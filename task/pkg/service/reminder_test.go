...
//			s.reminder.settings.Host+s.reminder.settings.Port,
//			s.testAuth,
//			s.reminder.settings.From,
//			[]string{task.UserEmail},
//			message).Return(nil)
//		err := s.reminder.RemindUserViaEmail(ctx)
//		s.NoError(err)
//		s.clientMock.AssertCalled(s.T(), "SendMail",
//			s.reminder.settings.Host,
//			s.testAuth,
//			s.reminder.settings.From,
//			[]string{task.UserEmail},
//		)
...
//		s.NoError(err)
//		s.Equalf(true, taskCheck.ReminderSent, "task %s of user %s", task.TaskID, task.UserEmail)
//	}
//}
